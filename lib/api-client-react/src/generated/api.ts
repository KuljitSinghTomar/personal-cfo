/**
 * Generated by orval v8.5.3 🍺
 * Do not edit manually.
 * Api
 * Family CFO API specification
 * OpenAPI spec version: 0.1.0
 */
import { useMutation, useQuery } from "@tanstack/react-query";
import type {
  MutationFunction,
  QueryFunction,
  QueryKey,
  UseMutationOptions,
  UseMutationResult,
  UseQueryOptions,
  UseQueryResult,
} from "@tanstack/react-query";

import type {
  AccountsResponse,
  AiChatBody,
  AiInsightsResponse,
  AutoGenerateResult,
  BudgetGoal,
  BudgetGoalBody,
  BudgetGoalsResponse,
  BudgetStatusResponse,
  CashflowResponse,
  CreateNetWorthAccountBody,
  DashboardSummary,
  DeleteBudgetGoal200,
  DeleteNetWorthAccount200,
  ForecastResponse,
  GetBudgetStatusParams,
  GetCashflowParams,
  GetDashboardSummaryParams,
  GetSpendingByCategoryParams,
  HealthStatus,
  ImportResult,
  ImportTransactionsBody,
  ListTransactionsParams,
  NetWorthAccount,
  NetWorthAccountsResponse,
  NetWorthHistory,
  NetWorthSnapshotRecord,
  NetWorthSummary,
  ScenarioBody,
  ScenarioResult,
  SpendingByCategoryResponse,
  SyncNetWorthAccounts200,
  Transaction,
  TransactionListResponse,
  UpdateBudgetGoalBody,
  UpdateNetWorthAccountBody,
  UpdateTransactionBody,
} from "./api.schemas";

import { customFetch } from "../custom-fetch";
import type { ErrorType, BodyType } from "../custom-fetch";

type AwaitedInput<T> = PromiseLike<T> | T;

type Awaited<O> = O extends AwaitedInput<infer T> ? T : never;

type SecondParameter<T extends (...args: never) => unknown> = Parameters<T>[1];

/**
 * @summary Health check
 */
export const getHealthCheckUrl = () => {
  return `/api/healthz`;
};

export const healthCheck = async (
  options?: RequestInit,
): Promise<HealthStatus> => {
  return customFetch<HealthStatus>(getHealthCheckUrl(), {
    ...options,
    method: "GET",
  });
};

export const getHealthCheckQueryKey = () => {
  return [`/api/healthz`] as const;
};

export const getHealthCheckQueryOptions = <
  TData = Awaited<ReturnType<typeof healthCheck>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof healthCheck>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getHealthCheckQueryKey();

  const queryFn: QueryFunction<Awaited<ReturnType<typeof healthCheck>>> = ({
    signal,
  }) => healthCheck({ signal, ...requestOptions });

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof healthCheck>>,
    TError,
    TData
  > & { queryKey: QueryKey };
};

export type HealthCheckQueryResult = NonNullable<
  Awaited<ReturnType<typeof healthCheck>>
>;
export type HealthCheckQueryError = ErrorType<unknown>;

/**
 * @summary Health check
 */

export function useHealthCheck<
  TData = Awaited<ReturnType<typeof healthCheck>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof healthCheck>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseQueryResult<TData, TError> & { queryKey: QueryKey } {
  const queryOptions = getHealthCheckQueryOptions(options);

  const query = useQuery(queryOptions) as UseQueryResult<TData, TError> & {
    queryKey: QueryKey;
  };

  return { ...query, queryKey: queryOptions.queryKey };
}

/**
 * @summary List transactions with filters
 */
export const getListTransactionsUrl = (params?: ListTransactionsParams) => {
  const normalizedParams = new URLSearchParams();

  Object.entries(params || {}).forEach(([key, value]) => {
    if (value !== undefined) {
      normalizedParams.append(key, value === null ? "null" : value.toString());
    }
  });

  const stringifiedParams = normalizedParams.toString();

  return stringifiedParams.length > 0
    ? `/api/transactions?${stringifiedParams}`
    : `/api/transactions`;
};

export const listTransactions = async (
  params?: ListTransactionsParams,
  options?: RequestInit,
): Promise<TransactionListResponse> => {
  return customFetch<TransactionListResponse>(getListTransactionsUrl(params), {
    ...options,
    method: "GET",
  });
};

export const getListTransactionsQueryKey = (
  params?: ListTransactionsParams,
) => {
  return [`/api/transactions`, ...(params ? [params] : [])] as const;
};

export const getListTransactionsQueryOptions = <
  TData = Awaited<ReturnType<typeof listTransactions>>,
  TError = ErrorType<unknown>,
>(
  params?: ListTransactionsParams,
  options?: {
    query?: UseQueryOptions<
      Awaited<ReturnType<typeof listTransactions>>,
      TError,
      TData
    >;
    request?: SecondParameter<typeof customFetch>;
  },
) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey =
    queryOptions?.queryKey ?? getListTransactionsQueryKey(params);

  const queryFn: QueryFunction<
    Awaited<ReturnType<typeof listTransactions>>
  > = ({ signal }) => listTransactions(params, { signal, ...requestOptions });

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof listTransactions>>,
    TError,
    TData
  > & { queryKey: QueryKey };
};

export type ListTransactionsQueryResult = NonNullable<
  Awaited<ReturnType<typeof listTransactions>>
>;
export type ListTransactionsQueryError = ErrorType<unknown>;

/**
 * @summary List transactions with filters
 */

export function useListTransactions<
  TData = Awaited<ReturnType<typeof listTransactions>>,
  TError = ErrorType<unknown>,
>(
  params?: ListTransactionsParams,
  options?: {
    query?: UseQueryOptions<
      Awaited<ReturnType<typeof listTransactions>>,
      TError,
      TData
    >;
    request?: SecondParameter<typeof customFetch>;
  },
): UseQueryResult<TData, TError> & { queryKey: QueryKey } {
  const queryOptions = getListTransactionsQueryOptions(params, options);

  const query = useQuery(queryOptions) as UseQueryResult<TData, TError> & {
    queryKey: QueryKey;
  };

  return { ...query, queryKey: queryOptions.queryKey };
}

/**
 * @summary Import transactions from Frollo CSV (raw CSV content as text)
 */
export const getImportTransactionsUrl = () => {
  return `/api/transactions/import`;
};

export const importTransactions = async (
  importTransactionsBody: ImportTransactionsBody,
  options?: RequestInit,
): Promise<ImportResult> => {
  return customFetch<ImportResult>(getImportTransactionsUrl(), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(importTransactionsBody),
  });
};

export const getImportTransactionsMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof importTransactions>>,
    TError,
    { data: BodyType<ImportTransactionsBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof importTransactions>>,
  TError,
  { data: BodyType<ImportTransactionsBody> },
  TContext
> => {
  const mutationKey = ["importTransactions"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof importTransactions>>,
    { data: BodyType<ImportTransactionsBody> }
  > = (props) => {
    const { data } = props ?? {};

    return importTransactions(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type ImportTransactionsMutationResult = NonNullable<
  Awaited<ReturnType<typeof importTransactions>>
>;
export type ImportTransactionsMutationBody = BodyType<ImportTransactionsBody>;
export type ImportTransactionsMutationError = ErrorType<unknown>;

/**
 * @summary Import transactions from Frollo CSV (raw CSV content as text)
 */
export const useImportTransactions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof importTransactions>>,
    TError,
    { data: BodyType<ImportTransactionsBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationResult<
  Awaited<ReturnType<typeof importTransactions>>,
  TError,
  { data: BodyType<ImportTransactionsBody> },
  TContext
> => {
  return useMutation(getImportTransactionsMutationOptions(options));
};

/**
 * @summary Update transaction (category, tags, isTransfer, isRecurring, userDescription)
 */
export const getUpdateTransactionUrl = (id: string) => {
  return `/api/transactions/${id}`;
};

export const updateTransaction = async (
  id: string,
  updateTransactionBody: UpdateTransactionBody,
  options?: RequestInit,
): Promise<Transaction> => {
  return customFetch<Transaction>(getUpdateTransactionUrl(id), {
    ...options,
    method: "PATCH",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(updateTransactionBody),
  });
};

export const getUpdateTransactionMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof updateTransaction>>,
    TError,
    { id: string; data: BodyType<UpdateTransactionBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof updateTransaction>>,
  TError,
  { id: string; data: BodyType<UpdateTransactionBody> },
  TContext
> => {
  const mutationKey = ["updateTransaction"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof updateTransaction>>,
    { id: string; data: BodyType<UpdateTransactionBody> }
  > = (props) => {
    const { id, data } = props ?? {};

    return updateTransaction(id, data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type UpdateTransactionMutationResult = NonNullable<
  Awaited<ReturnType<typeof updateTransaction>>
>;
export type UpdateTransactionMutationBody = BodyType<UpdateTransactionBody>;
export type UpdateTransactionMutationError = ErrorType<unknown>;

/**
 * @summary Update transaction (category, tags, isTransfer, isRecurring, userDescription)
 */
export const useUpdateTransaction = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof updateTransaction>>,
    TError,
    { id: string; data: BodyType<UpdateTransactionBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationResult<
  Awaited<ReturnType<typeof updateTransaction>>,
  TError,
  { id: string; data: BodyType<UpdateTransactionBody> },
  TContext
> => {
  return useMutation(getUpdateTransactionMutationOptions(options));
};

/**
 * @summary Get financial summary for date range
 */
export const getGetDashboardSummaryUrl = (
  params?: GetDashboardSummaryParams,
) => {
  const normalizedParams = new URLSearchParams();

  Object.entries(params || {}).forEach(([key, value]) => {
    if (value !== undefined) {
      normalizedParams.append(key, value === null ? "null" : value.toString());
    }
  });

  const stringifiedParams = normalizedParams.toString();

  return stringifiedParams.length > 0
    ? `/api/dashboard/summary?${stringifiedParams}`
    : `/api/dashboard/summary`;
};

export const getDashboardSummary = async (
  params?: GetDashboardSummaryParams,
  options?: RequestInit,
): Promise<DashboardSummary> => {
  return customFetch<DashboardSummary>(getGetDashboardSummaryUrl(params), {
    ...options,
    method: "GET",
  });
};

export const getGetDashboardSummaryQueryKey = (
  params?: GetDashboardSummaryParams,
) => {
  return [`/api/dashboard/summary`, ...(params ? [params] : [])] as const;
};

export const getGetDashboardSummaryQueryOptions = <
  TData = Awaited<ReturnType<typeof getDashboardSummary>>,
  TError = ErrorType<unknown>,
>(
  params?: GetDashboardSummaryParams,
  options?: {
    query?: UseQueryOptions<
      Awaited<ReturnType<typeof getDashboardSummary>>,
      TError,
      TData
    >;
    request?: SecondParameter<typeof customFetch>;
  },
) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey =
    queryOptions?.queryKey ?? getGetDashboardSummaryQueryKey(params);

  const queryFn: QueryFunction<
    Awaited<ReturnType<typeof getDashboardSummary>>
  > = ({ signal }) =>
    getDashboardSummary(params, { signal, ...requestOptions });

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof getDashboardSummary>>,
    TError,
    TData
  > & { queryKey: QueryKey };
};

export type GetDashboardSummaryQueryResult = NonNullable<
  Awaited<ReturnType<typeof getDashboardSummary>>
>;
export type GetDashboardSummaryQueryError = ErrorType<unknown>;

/**
 * @summary Get financial summary for date range
 */

export function useGetDashboardSummary<
  TData = Awaited<ReturnType<typeof getDashboardSummary>>,
  TError = ErrorType<unknown>,
>(
  params?: GetDashboardSummaryParams,
  options?: {
    query?: UseQueryOptions<
      Awaited<ReturnType<typeof getDashboardSummary>>,
      TError,
      TData
    >;
    request?: SecondParameter<typeof customFetch>;
  },
): UseQueryResult<TData, TError> & { queryKey: QueryKey } {
  const queryOptions = getGetDashboardSummaryQueryOptions(params, options);

  const query = useQuery(queryOptions) as UseQueryResult<TData, TError> & {
    queryKey: QueryKey;
  };

  return { ...query, queryKey: queryOptions.queryKey };
}

/**
 * @summary Get cash flow data for chart (monthly breakdown)
 */
export const getGetCashflowUrl = (params?: GetCashflowParams) => {
  const normalizedParams = new URLSearchParams();

  Object.entries(params || {}).forEach(([key, value]) => {
    if (value !== undefined) {
      normalizedParams.append(key, value === null ? "null" : value.toString());
    }
  });

  const stringifiedParams = normalizedParams.toString();

  return stringifiedParams.length > 0
    ? `/api/dashboard/cashflow?${stringifiedParams}`
    : `/api/dashboard/cashflow`;
};

export const getCashflow = async (
  params?: GetCashflowParams,
  options?: RequestInit,
): Promise<CashflowResponse> => {
  return customFetch<CashflowResponse>(getGetCashflowUrl(params), {
    ...options,
    method: "GET",
  });
};

export const getGetCashflowQueryKey = (params?: GetCashflowParams) => {
  return [`/api/dashboard/cashflow`, ...(params ? [params] : [])] as const;
};

export const getGetCashflowQueryOptions = <
  TData = Awaited<ReturnType<typeof getCashflow>>,
  TError = ErrorType<unknown>,
>(
  params?: GetCashflowParams,
  options?: {
    query?: UseQueryOptions<
      Awaited<ReturnType<typeof getCashflow>>,
      TError,
      TData
    >;
    request?: SecondParameter<typeof customFetch>;
  },
) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getGetCashflowQueryKey(params);

  const queryFn: QueryFunction<Awaited<ReturnType<typeof getCashflow>>> = ({
    signal,
  }) => getCashflow(params, { signal, ...requestOptions });

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof getCashflow>>,
    TError,
    TData
  > & { queryKey: QueryKey };
};

export type GetCashflowQueryResult = NonNullable<
  Awaited<ReturnType<typeof getCashflow>>
>;
export type GetCashflowQueryError = ErrorType<unknown>;

/**
 * @summary Get cash flow data for chart (monthly breakdown)
 */

export function useGetCashflow<
  TData = Awaited<ReturnType<typeof getCashflow>>,
  TError = ErrorType<unknown>,
>(
  params?: GetCashflowParams,
  options?: {
    query?: UseQueryOptions<
      Awaited<ReturnType<typeof getCashflow>>,
      TError,
      TData
    >;
    request?: SecondParameter<typeof customFetch>;
  },
): UseQueryResult<TData, TError> & { queryKey: QueryKey } {
  const queryOptions = getGetCashflowQueryOptions(params, options);

  const query = useQuery(queryOptions) as UseQueryResult<TData, TError> & {
    queryKey: QueryKey;
  };

  return { ...query, queryKey: queryOptions.queryKey };
}

/**
 * @summary Get spending breakdown by category
 */
export const getGetSpendingByCategoryUrl = (
  params?: GetSpendingByCategoryParams,
) => {
  const normalizedParams = new URLSearchParams();

  Object.entries(params || {}).forEach(([key, value]) => {
    if (value !== undefined) {
      normalizedParams.append(key, value === null ? "null" : value.toString());
    }
  });

  const stringifiedParams = normalizedParams.toString();

  return stringifiedParams.length > 0
    ? `/api/dashboard/spending-by-category?${stringifiedParams}`
    : `/api/dashboard/spending-by-category`;
};

export const getSpendingByCategory = async (
  params?: GetSpendingByCategoryParams,
  options?: RequestInit,
): Promise<SpendingByCategoryResponse> => {
  return customFetch<SpendingByCategoryResponse>(
    getGetSpendingByCategoryUrl(params),
    {
      ...options,
      method: "GET",
    },
  );
};

export const getGetSpendingByCategoryQueryKey = (
  params?: GetSpendingByCategoryParams,
) => {
  return [
    `/api/dashboard/spending-by-category`,
    ...(params ? [params] : []),
  ] as const;
};

export const getGetSpendingByCategoryQueryOptions = <
  TData = Awaited<ReturnType<typeof getSpendingByCategory>>,
  TError = ErrorType<unknown>,
>(
  params?: GetSpendingByCategoryParams,
  options?: {
    query?: UseQueryOptions<
      Awaited<ReturnType<typeof getSpendingByCategory>>,
      TError,
      TData
    >;
    request?: SecondParameter<typeof customFetch>;
  },
) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey =
    queryOptions?.queryKey ?? getGetSpendingByCategoryQueryKey(params);

  const queryFn: QueryFunction<
    Awaited<ReturnType<typeof getSpendingByCategory>>
  > = ({ signal }) =>
    getSpendingByCategory(params, { signal, ...requestOptions });

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof getSpendingByCategory>>,
    TError,
    TData
  > & { queryKey: QueryKey };
};

export type GetSpendingByCategoryQueryResult = NonNullable<
  Awaited<ReturnType<typeof getSpendingByCategory>>
>;
export type GetSpendingByCategoryQueryError = ErrorType<unknown>;

/**
 * @summary Get spending breakdown by category
 */

export function useGetSpendingByCategory<
  TData = Awaited<ReturnType<typeof getSpendingByCategory>>,
  TError = ErrorType<unknown>,
>(
  params?: GetSpendingByCategoryParams,
  options?: {
    query?: UseQueryOptions<
      Awaited<ReturnType<typeof getSpendingByCategory>>,
      TError,
      TData
    >;
    request?: SecondParameter<typeof customFetch>;
  },
): UseQueryResult<TData, TError> & { queryKey: QueryKey } {
  const queryOptions = getGetSpendingByCategoryQueryOptions(params, options);

  const query = useQuery(queryOptions) as UseQueryResult<TData, TError> & {
    queryKey: QueryKey;
  };

  return { ...query, queryKey: queryOptions.queryKey };
}

/**
 * @summary List all accounts with balance summary
 */
export const getGetAccountsUrl = () => {
  return `/api/dashboard/accounts`;
};

export const getAccounts = async (
  options?: RequestInit,
): Promise<AccountsResponse> => {
  return customFetch<AccountsResponse>(getGetAccountsUrl(), {
    ...options,
    method: "GET",
  });
};

export const getGetAccountsQueryKey = () => {
  return [`/api/dashboard/accounts`] as const;
};

export const getGetAccountsQueryOptions = <
  TData = Awaited<ReturnType<typeof getAccounts>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof getAccounts>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getGetAccountsQueryKey();

  const queryFn: QueryFunction<Awaited<ReturnType<typeof getAccounts>>> = ({
    signal,
  }) => getAccounts({ signal, ...requestOptions });

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof getAccounts>>,
    TError,
    TData
  > & { queryKey: QueryKey };
};

export type GetAccountsQueryResult = NonNullable<
  Awaited<ReturnType<typeof getAccounts>>
>;
export type GetAccountsQueryError = ErrorType<unknown>;

/**
 * @summary List all accounts with balance summary
 */

export function useGetAccounts<
  TData = Awaited<ReturnType<typeof getAccounts>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof getAccounts>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseQueryResult<TData, TError> & { queryKey: QueryKey } {
  const queryOptions = getGetAccountsQueryOptions(options);

  const query = useQuery(queryOptions) as UseQueryResult<TData, TError> & {
    queryKey: QueryKey;
  };

  return { ...query, queryKey: queryOptions.queryKey };
}

/**
 * @summary Get end-of-month savings forecast
 */
export const getGetForecastUrl = () => {
  return `/api/dashboard/forecast`;
};

export const getForecast = async (
  options?: RequestInit,
): Promise<ForecastResponse> => {
  return customFetch<ForecastResponse>(getGetForecastUrl(), {
    ...options,
    method: "GET",
  });
};

export const getGetForecastQueryKey = () => {
  return [`/api/dashboard/forecast`] as const;
};

export const getGetForecastQueryOptions = <
  TData = Awaited<ReturnType<typeof getForecast>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof getForecast>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getGetForecastQueryKey();

  const queryFn: QueryFunction<Awaited<ReturnType<typeof getForecast>>> = ({
    signal,
  }) => getForecast({ signal, ...requestOptions });

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof getForecast>>,
    TError,
    TData
  > & { queryKey: QueryKey };
};

export type GetForecastQueryResult = NonNullable<
  Awaited<ReturnType<typeof getForecast>>
>;
export type GetForecastQueryError = ErrorType<unknown>;

/**
 * @summary Get end-of-month savings forecast
 */

export function useGetForecast<
  TData = Awaited<ReturnType<typeof getForecast>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof getForecast>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseQueryResult<TData, TError> & { queryKey: QueryKey } {
  const queryOptions = getGetForecastQueryOptions(options);

  const query = useQuery(queryOptions) as UseQueryResult<TData, TError> & {
    queryKey: QueryKey;
  };

  return { ...query, queryKey: queryOptions.queryKey };
}

/**
 * @summary Get AI-generated financial insights
 */
export const getGetAiInsightsUrl = () => {
  return `/api/ai/insights`;
};

export const getAiInsights = async (
  options?: RequestInit,
): Promise<AiInsightsResponse> => {
  return customFetch<AiInsightsResponse>(getGetAiInsightsUrl(), {
    ...options,
    method: "GET",
  });
};

export const getGetAiInsightsQueryKey = () => {
  return [`/api/ai/insights`] as const;
};

export const getGetAiInsightsQueryOptions = <
  TData = Awaited<ReturnType<typeof getAiInsights>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof getAiInsights>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getGetAiInsightsQueryKey();

  const queryFn: QueryFunction<Awaited<ReturnType<typeof getAiInsights>>> = ({
    signal,
  }) => getAiInsights({ signal, ...requestOptions });

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof getAiInsights>>,
    TError,
    TData
  > & { queryKey: QueryKey };
};

export type GetAiInsightsQueryResult = NonNullable<
  Awaited<ReturnType<typeof getAiInsights>>
>;
export type GetAiInsightsQueryError = ErrorType<unknown>;

/**
 * @summary Get AI-generated financial insights
 */

export function useGetAiInsights<
  TData = Awaited<ReturnType<typeof getAiInsights>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof getAiInsights>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseQueryResult<TData, TError> & { queryKey: QueryKey } {
  const queryOptions = getGetAiInsightsQueryOptions(options);

  const query = useQuery(queryOptions) as UseQueryResult<TData, TError> & {
    queryKey: QueryKey;
  };

  return { ...query, queryKey: queryOptions.queryKey };
}

/**
 * @summary Send a message to the AI financial advisor (streaming SSE)
 */
export const getSendAiMessageUrl = () => {
  return `/api/ai/chat`;
};

export const sendAiMessage = async (
  aiChatBody: AiChatBody,
  options?: RequestInit,
): Promise<string> => {
  return customFetch<string>(getSendAiMessageUrl(), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(aiChatBody),
  });
};

export const getSendAiMessageMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof sendAiMessage>>,
    TError,
    { data: BodyType<AiChatBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof sendAiMessage>>,
  TError,
  { data: BodyType<AiChatBody> },
  TContext
> => {
  const mutationKey = ["sendAiMessage"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof sendAiMessage>>,
    { data: BodyType<AiChatBody> }
  > = (props) => {
    const { data } = props ?? {};

    return sendAiMessage(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type SendAiMessageMutationResult = NonNullable<
  Awaited<ReturnType<typeof sendAiMessage>>
>;
export type SendAiMessageMutationBody = BodyType<AiChatBody>;
export type SendAiMessageMutationError = ErrorType<unknown>;

/**
 * @summary Send a message to the AI financial advisor (streaming SSE)
 */
export const useSendAiMessage = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof sendAiMessage>>,
    TError,
    { data: BodyType<AiChatBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationResult<
  Awaited<ReturnType<typeof sendAiMessage>>,
  TError,
  { data: BodyType<AiChatBody> },
  TContext
> => {
  return useMutation(getSendAiMessageMutationOptions(options));
};

/**
 * @summary Simulate a financial scenario
 */
export const getSimulateScenarioUrl = () => {
  return `/api/scenarios/simulate`;
};

export const simulateScenario = async (
  scenarioBody: ScenarioBody,
  options?: RequestInit,
): Promise<ScenarioResult> => {
  return customFetch<ScenarioResult>(getSimulateScenarioUrl(), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(scenarioBody),
  });
};

export const getSimulateScenarioMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof simulateScenario>>,
    TError,
    { data: BodyType<ScenarioBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof simulateScenario>>,
  TError,
  { data: BodyType<ScenarioBody> },
  TContext
> => {
  const mutationKey = ["simulateScenario"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof simulateScenario>>,
    { data: BodyType<ScenarioBody> }
  > = (props) => {
    const { data } = props ?? {};

    return simulateScenario(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type SimulateScenarioMutationResult = NonNullable<
  Awaited<ReturnType<typeof simulateScenario>>
>;
export type SimulateScenarioMutationBody = BodyType<ScenarioBody>;
export type SimulateScenarioMutationError = ErrorType<unknown>;

/**
 * @summary Simulate a financial scenario
 */
export const useSimulateScenario = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof simulateScenario>>,
    TError,
    { data: BodyType<ScenarioBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationResult<
  Awaited<ReturnType<typeof simulateScenario>>,
  TError,
  { data: BodyType<ScenarioBody> },
  TContext
> => {
  return useMutation(getSimulateScenarioMutationOptions(options));
};

/**
 * @summary List all budget goals
 */
export const getListBudgetGoalsUrl = () => {
  return `/api/budget/goals`;
};

export const listBudgetGoals = async (
  options?: RequestInit,
): Promise<BudgetGoalsResponse> => {
  return customFetch<BudgetGoalsResponse>(getListBudgetGoalsUrl(), {
    ...options,
    method: "GET",
  });
};

export const getListBudgetGoalsQueryKey = () => {
  return [`/api/budget/goals`] as const;
};

export const getListBudgetGoalsQueryOptions = <
  TData = Awaited<ReturnType<typeof listBudgetGoals>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof listBudgetGoals>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getListBudgetGoalsQueryKey();

  const queryFn: QueryFunction<Awaited<ReturnType<typeof listBudgetGoals>>> = ({
    signal,
  }) => listBudgetGoals({ signal, ...requestOptions });

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof listBudgetGoals>>,
    TError,
    TData
  > & { queryKey: QueryKey };
};

export type ListBudgetGoalsQueryResult = NonNullable<
  Awaited<ReturnType<typeof listBudgetGoals>>
>;
export type ListBudgetGoalsQueryError = ErrorType<unknown>;

/**
 * @summary List all budget goals
 */

export function useListBudgetGoals<
  TData = Awaited<ReturnType<typeof listBudgetGoals>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof listBudgetGoals>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseQueryResult<TData, TError> & { queryKey: QueryKey } {
  const queryOptions = getListBudgetGoalsQueryOptions(options);

  const query = useQuery(queryOptions) as UseQueryResult<TData, TError> & {
    queryKey: QueryKey;
  };

  return { ...query, queryKey: queryOptions.queryKey };
}

/**
 * @summary Create or update a budget goal for a category
 */
export const getCreateBudgetGoalUrl = () => {
  return `/api/budget/goals`;
};

export const createBudgetGoal = async (
  budgetGoalBody: BudgetGoalBody,
  options?: RequestInit,
): Promise<BudgetGoal> => {
  return customFetch<BudgetGoal>(getCreateBudgetGoalUrl(), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(budgetGoalBody),
  });
};

export const getCreateBudgetGoalMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof createBudgetGoal>>,
    TError,
    { data: BodyType<BudgetGoalBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof createBudgetGoal>>,
  TError,
  { data: BodyType<BudgetGoalBody> },
  TContext
> => {
  const mutationKey = ["createBudgetGoal"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof createBudgetGoal>>,
    { data: BodyType<BudgetGoalBody> }
  > = (props) => {
    const { data } = props ?? {};

    return createBudgetGoal(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type CreateBudgetGoalMutationResult = NonNullable<
  Awaited<ReturnType<typeof createBudgetGoal>>
>;
export type CreateBudgetGoalMutationBody = BodyType<BudgetGoalBody>;
export type CreateBudgetGoalMutationError = ErrorType<unknown>;

/**
 * @summary Create or update a budget goal for a category
 */
export const useCreateBudgetGoal = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof createBudgetGoal>>,
    TError,
    { data: BodyType<BudgetGoalBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationResult<
  Awaited<ReturnType<typeof createBudgetGoal>>,
  TError,
  { data: BodyType<BudgetGoalBody> },
  TContext
> => {
  return useMutation(getCreateBudgetGoalMutationOptions(options));
};

/**
 * @summary Update a budget goal's monthly limit (marks it as user-edited)
 */
export const getUpdateBudgetGoalUrl = (id: string) => {
  return `/api/budget/goals/${id}`;
};

export const updateBudgetGoal = async (
  id: string,
  updateBudgetGoalBody: UpdateBudgetGoalBody,
  options?: RequestInit,
): Promise<BudgetGoal> => {
  return customFetch<BudgetGoal>(getUpdateBudgetGoalUrl(id), {
    ...options,
    method: "PUT",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(updateBudgetGoalBody),
  });
};

export const getUpdateBudgetGoalMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof updateBudgetGoal>>,
    TError,
    { id: string; data: BodyType<UpdateBudgetGoalBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof updateBudgetGoal>>,
  TError,
  { id: string; data: BodyType<UpdateBudgetGoalBody> },
  TContext
> => {
  const mutationKey = ["updateBudgetGoal"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof updateBudgetGoal>>,
    { id: string; data: BodyType<UpdateBudgetGoalBody> }
  > = (props) => {
    const { id, data } = props ?? {};

    return updateBudgetGoal(id, data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type UpdateBudgetGoalMutationResult = NonNullable<
  Awaited<ReturnType<typeof updateBudgetGoal>>
>;
export type UpdateBudgetGoalMutationBody = BodyType<UpdateBudgetGoalBody>;
export type UpdateBudgetGoalMutationError = ErrorType<unknown>;

/**
 * @summary Update a budget goal's monthly limit (marks it as user-edited)
 */
export const useUpdateBudgetGoal = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof updateBudgetGoal>>,
    TError,
    { id: string; data: BodyType<UpdateBudgetGoalBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationResult<
  Awaited<ReturnType<typeof updateBudgetGoal>>,
  TError,
  { id: string; data: BodyType<UpdateBudgetGoalBody> },
  TContext
> => {
  return useMutation(getUpdateBudgetGoalMutationOptions(options));
};

/**
 * @summary Delete a budget goal
 */
export const getDeleteBudgetGoalUrl = (id: string) => {
  return `/api/budget/goals/${id}`;
};

export const deleteBudgetGoal = async (
  id: string,
  options?: RequestInit,
): Promise<DeleteBudgetGoal200> => {
  return customFetch<DeleteBudgetGoal200>(getDeleteBudgetGoalUrl(id), {
    ...options,
    method: "DELETE",
  });
};

export const getDeleteBudgetGoalMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof deleteBudgetGoal>>,
    TError,
    { id: string },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof deleteBudgetGoal>>,
  TError,
  { id: string },
  TContext
> => {
  const mutationKey = ["deleteBudgetGoal"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof deleteBudgetGoal>>,
    { id: string }
  > = (props) => {
    const { id } = props ?? {};

    return deleteBudgetGoal(id, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type DeleteBudgetGoalMutationResult = NonNullable<
  Awaited<ReturnType<typeof deleteBudgetGoal>>
>;

export type DeleteBudgetGoalMutationError = ErrorType<unknown>;

/**
 * @summary Delete a budget goal
 */
export const useDeleteBudgetGoal = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof deleteBudgetGoal>>,
    TError,
    { id: string },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationResult<
  Awaited<ReturnType<typeof deleteBudgetGoal>>,
  TError,
  { id: string },
  TContext
> => {
  return useMutation(getDeleteBudgetGoalMutationOptions(options));
};

/**
 * @summary Auto-generate budget goals from last 12 months of transaction history
 */
export const getAutoGenerateBudgetsUrl = () => {
  return `/api/budget/auto-generate`;
};

export const autoGenerateBudgets = async (
  options?: RequestInit,
): Promise<AutoGenerateResult> => {
  return customFetch<AutoGenerateResult>(getAutoGenerateBudgetsUrl(), {
    ...options,
    method: "POST",
  });
};

export const getAutoGenerateBudgetsMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof autoGenerateBudgets>>,
    TError,
    void,
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof autoGenerateBudgets>>,
  TError,
  void,
  TContext
> => {
  const mutationKey = ["autoGenerateBudgets"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof autoGenerateBudgets>>,
    void
  > = () => {
    return autoGenerateBudgets(requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type AutoGenerateBudgetsMutationResult = NonNullable<
  Awaited<ReturnType<typeof autoGenerateBudgets>>
>;

export type AutoGenerateBudgetsMutationError = ErrorType<unknown>;

/**
 * @summary Auto-generate budget goals from last 12 months of transaction history
 */
export const useAutoGenerateBudgets = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof autoGenerateBudgets>>,
    TError,
    void,
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationResult<
  Awaited<ReturnType<typeof autoGenerateBudgets>>,
  TError,
  void,
  TContext
> => {
  return useMutation(getAutoGenerateBudgetsMutationOptions(options));
};

/**
 * @summary Get current month budget status (goals vs actuals)
 */
export const getGetBudgetStatusUrl = (params?: GetBudgetStatusParams) => {
  const normalizedParams = new URLSearchParams();

  Object.entries(params || {}).forEach(([key, value]) => {
    if (value !== undefined) {
      normalizedParams.append(key, value === null ? "null" : value.toString());
    }
  });

  const stringifiedParams = normalizedParams.toString();

  return stringifiedParams.length > 0
    ? `/api/budget/status?${stringifiedParams}`
    : `/api/budget/status`;
};

export const getBudgetStatus = async (
  params?: GetBudgetStatusParams,
  options?: RequestInit,
): Promise<BudgetStatusResponse> => {
  return customFetch<BudgetStatusResponse>(getGetBudgetStatusUrl(params), {
    ...options,
    method: "GET",
  });
};

export const getGetBudgetStatusQueryKey = (params?: GetBudgetStatusParams) => {
  return [`/api/budget/status`, ...(params ? [params] : [])] as const;
};

export const getGetBudgetStatusQueryOptions = <
  TData = Awaited<ReturnType<typeof getBudgetStatus>>,
  TError = ErrorType<unknown>,
>(
  params?: GetBudgetStatusParams,
  options?: {
    query?: UseQueryOptions<
      Awaited<ReturnType<typeof getBudgetStatus>>,
      TError,
      TData
    >;
    request?: SecondParameter<typeof customFetch>;
  },
) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getGetBudgetStatusQueryKey(params);

  const queryFn: QueryFunction<Awaited<ReturnType<typeof getBudgetStatus>>> = ({
    signal,
  }) => getBudgetStatus(params, { signal, ...requestOptions });

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof getBudgetStatus>>,
    TError,
    TData
  > & { queryKey: QueryKey };
};

export type GetBudgetStatusQueryResult = NonNullable<
  Awaited<ReturnType<typeof getBudgetStatus>>
>;
export type GetBudgetStatusQueryError = ErrorType<unknown>;

/**
 * @summary Get current month budget status (goals vs actuals)
 */

export function useGetBudgetStatus<
  TData = Awaited<ReturnType<typeof getBudgetStatus>>,
  TError = ErrorType<unknown>,
>(
  params?: GetBudgetStatusParams,
  options?: {
    query?: UseQueryOptions<
      Awaited<ReturnType<typeof getBudgetStatus>>,
      TError,
      TData
    >;
    request?: SecondParameter<typeof customFetch>;
  },
): UseQueryResult<TData, TError> & { queryKey: QueryKey } {
  const queryOptions = getGetBudgetStatusQueryOptions(params, options);

  const query = useQuery(queryOptions) as UseQueryResult<TData, TError> & {
    queryKey: QueryKey;
  };

  return { ...query, queryKey: queryOptions.queryKey };
}

/**
 * @summary List all net worth accounts
 */
export const getListNetWorthAccountsUrl = () => {
  return `/api/net-worth/accounts`;
};

export const listNetWorthAccounts = async (
  options?: RequestInit,
): Promise<NetWorthAccountsResponse> => {
  return customFetch<NetWorthAccountsResponse>(getListNetWorthAccountsUrl(), {
    ...options,
    method: "GET",
  });
};

export const getListNetWorthAccountsQueryKey = () => {
  return [`/api/net-worth/accounts`] as const;
};

export const getListNetWorthAccountsQueryOptions = <
  TData = Awaited<ReturnType<typeof listNetWorthAccounts>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof listNetWorthAccounts>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getListNetWorthAccountsQueryKey();

  const queryFn: QueryFunction<
    Awaited<ReturnType<typeof listNetWorthAccounts>>
  > = ({ signal }) => listNetWorthAccounts({ signal, ...requestOptions });

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof listNetWorthAccounts>>,
    TError,
    TData
  > & { queryKey: QueryKey };
};

export type ListNetWorthAccountsQueryResult = NonNullable<
  Awaited<ReturnType<typeof listNetWorthAccounts>>
>;
export type ListNetWorthAccountsQueryError = ErrorType<unknown>;

/**
 * @summary List all net worth accounts
 */

export function useListNetWorthAccounts<
  TData = Awaited<ReturnType<typeof listNetWorthAccounts>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof listNetWorthAccounts>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseQueryResult<TData, TError> & { queryKey: QueryKey } {
  const queryOptions = getListNetWorthAccountsQueryOptions(options);

  const query = useQuery(queryOptions) as UseQueryResult<TData, TError> & {
    queryKey: QueryKey;
  };

  return { ...query, queryKey: queryOptions.queryKey };
}

/**
 * @summary Create a net worth account
 */
export const getCreateNetWorthAccountUrl = () => {
  return `/api/net-worth/accounts`;
};

export const createNetWorthAccount = async (
  createNetWorthAccountBody: CreateNetWorthAccountBody,
  options?: RequestInit,
): Promise<NetWorthAccount> => {
  return customFetch<NetWorthAccount>(getCreateNetWorthAccountUrl(), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(createNetWorthAccountBody),
  });
};

export const getCreateNetWorthAccountMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof createNetWorthAccount>>,
    TError,
    { data: BodyType<CreateNetWorthAccountBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof createNetWorthAccount>>,
  TError,
  { data: BodyType<CreateNetWorthAccountBody> },
  TContext
> => {
  const mutationKey = ["createNetWorthAccount"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof createNetWorthAccount>>,
    { data: BodyType<CreateNetWorthAccountBody> }
  > = (props) => {
    const { data } = props ?? {};

    return createNetWorthAccount(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type CreateNetWorthAccountMutationResult = NonNullable<
  Awaited<ReturnType<typeof createNetWorthAccount>>
>;
export type CreateNetWorthAccountMutationBody =
  BodyType<CreateNetWorthAccountBody>;
export type CreateNetWorthAccountMutationError = ErrorType<unknown>;

/**
 * @summary Create a net worth account
 */
export const useCreateNetWorthAccount = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof createNetWorthAccount>>,
    TError,
    { data: BodyType<CreateNetWorthAccountBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationResult<
  Awaited<ReturnType<typeof createNetWorthAccount>>,
  TError,
  { data: BodyType<CreateNetWorthAccountBody> },
  TContext
> => {
  return useMutation(getCreateNetWorthAccountMutationOptions(options));
};

/**
 * @summary Update a net worth account
 */
export const getUpdateNetWorthAccountUrl = (id: string) => {
  return `/api/net-worth/accounts/${id}`;
};

export const updateNetWorthAccount = async (
  id: string,
  updateNetWorthAccountBody: UpdateNetWorthAccountBody,
  options?: RequestInit,
): Promise<NetWorthAccount> => {
  return customFetch<NetWorthAccount>(getUpdateNetWorthAccountUrl(id), {
    ...options,
    method: "PUT",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(updateNetWorthAccountBody),
  });
};

export const getUpdateNetWorthAccountMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof updateNetWorthAccount>>,
    TError,
    { id: string; data: BodyType<UpdateNetWorthAccountBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof updateNetWorthAccount>>,
  TError,
  { id: string; data: BodyType<UpdateNetWorthAccountBody> },
  TContext
> => {
  const mutationKey = ["updateNetWorthAccount"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof updateNetWorthAccount>>,
    { id: string; data: BodyType<UpdateNetWorthAccountBody> }
  > = (props) => {
    const { id, data } = props ?? {};

    return updateNetWorthAccount(id, data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type UpdateNetWorthAccountMutationResult = NonNullable<
  Awaited<ReturnType<typeof updateNetWorthAccount>>
>;
export type UpdateNetWorthAccountMutationBody =
  BodyType<UpdateNetWorthAccountBody>;
export type UpdateNetWorthAccountMutationError = ErrorType<unknown>;

/**
 * @summary Update a net worth account
 */
export const useUpdateNetWorthAccount = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof updateNetWorthAccount>>,
    TError,
    { id: string; data: BodyType<UpdateNetWorthAccountBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationResult<
  Awaited<ReturnType<typeof updateNetWorthAccount>>,
  TError,
  { id: string; data: BodyType<UpdateNetWorthAccountBody> },
  TContext
> => {
  return useMutation(getUpdateNetWorthAccountMutationOptions(options));
};

/**
 * @summary Delete a net worth account
 */
export const getDeleteNetWorthAccountUrl = (id: string) => {
  return `/api/net-worth/accounts/${id}`;
};

export const deleteNetWorthAccount = async (
  id: string,
  options?: RequestInit,
): Promise<DeleteNetWorthAccount200> => {
  return customFetch<DeleteNetWorthAccount200>(
    getDeleteNetWorthAccountUrl(id),
    {
      ...options,
      method: "DELETE",
    },
  );
};

export const getDeleteNetWorthAccountMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof deleteNetWorthAccount>>,
    TError,
    { id: string },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof deleteNetWorthAccount>>,
  TError,
  { id: string },
  TContext
> => {
  const mutationKey = ["deleteNetWorthAccount"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof deleteNetWorthAccount>>,
    { id: string }
  > = (props) => {
    const { id } = props ?? {};

    return deleteNetWorthAccount(id, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type DeleteNetWorthAccountMutationResult = NonNullable<
  Awaited<ReturnType<typeof deleteNetWorthAccount>>
>;

export type DeleteNetWorthAccountMutationError = ErrorType<unknown>;

/**
 * @summary Delete a net worth account
 */
export const useDeleteNetWorthAccount = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof deleteNetWorthAccount>>,
    TError,
    { id: string },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationResult<
  Awaited<ReturnType<typeof deleteNetWorthAccount>>,
  TError,
  { id: string },
  TContext
> => {
  return useMutation(getDeleteNetWorthAccountMutationOptions(options));
};

/**
 * @summary Current net worth summary
 */
export const getGetNetWorthSummaryUrl = () => {
  return `/api/net-worth/summary`;
};

export const getNetWorthSummary = async (
  options?: RequestInit,
): Promise<NetWorthSummary> => {
  return customFetch<NetWorthSummary>(getGetNetWorthSummaryUrl(), {
    ...options,
    method: "GET",
  });
};

export const getGetNetWorthSummaryQueryKey = () => {
  return [`/api/net-worth/summary`] as const;
};

export const getGetNetWorthSummaryQueryOptions = <
  TData = Awaited<ReturnType<typeof getNetWorthSummary>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof getNetWorthSummary>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getGetNetWorthSummaryQueryKey();

  const queryFn: QueryFunction<
    Awaited<ReturnType<typeof getNetWorthSummary>>
  > = ({ signal }) => getNetWorthSummary({ signal, ...requestOptions });

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof getNetWorthSummary>>,
    TError,
    TData
  > & { queryKey: QueryKey };
};

export type GetNetWorthSummaryQueryResult = NonNullable<
  Awaited<ReturnType<typeof getNetWorthSummary>>
>;
export type GetNetWorthSummaryQueryError = ErrorType<unknown>;

/**
 * @summary Current net worth summary
 */

export function useGetNetWorthSummary<
  TData = Awaited<ReturnType<typeof getNetWorthSummary>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof getNetWorthSummary>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseQueryResult<TData, TError> & { queryKey: QueryKey } {
  const queryOptions = getGetNetWorthSummaryQueryOptions(options);

  const query = useQuery(queryOptions) as UseQueryResult<TData, TError> & {
    queryKey: QueryKey;
  };

  return { ...query, queryKey: queryOptions.queryKey };
}

/**
 * @summary Net worth history snapshots
 */
export const getGetNetWorthHistoryUrl = () => {
  return `/api/net-worth/history`;
};

export const getNetWorthHistory = async (
  options?: RequestInit,
): Promise<NetWorthHistory> => {
  return customFetch<NetWorthHistory>(getGetNetWorthHistoryUrl(), {
    ...options,
    method: "GET",
  });
};

export const getGetNetWorthHistoryQueryKey = () => {
  return [`/api/net-worth/history`] as const;
};

export const getGetNetWorthHistoryQueryOptions = <
  TData = Awaited<ReturnType<typeof getNetWorthHistory>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof getNetWorthHistory>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getGetNetWorthHistoryQueryKey();

  const queryFn: QueryFunction<
    Awaited<ReturnType<typeof getNetWorthHistory>>
  > = ({ signal }) => getNetWorthHistory({ signal, ...requestOptions });

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof getNetWorthHistory>>,
    TError,
    TData
  > & { queryKey: QueryKey };
};

export type GetNetWorthHistoryQueryResult = NonNullable<
  Awaited<ReturnType<typeof getNetWorthHistory>>
>;
export type GetNetWorthHistoryQueryError = ErrorType<unknown>;

/**
 * @summary Net worth history snapshots
 */

export function useGetNetWorthHistory<
  TData = Awaited<ReturnType<typeof getNetWorthHistory>>,
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<
    Awaited<ReturnType<typeof getNetWorthHistory>>,
    TError,
    TData
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseQueryResult<TData, TError> & { queryKey: QueryKey } {
  const queryOptions = getGetNetWorthHistoryQueryOptions(options);

  const query = useQuery(queryOptions) as UseQueryResult<TData, TError> & {
    queryKey: QueryKey;
  };

  return { ...query, queryKey: queryOptions.queryKey };
}

/**
 * @summary Sync derived account balances from transactions
 */
export const getSyncNetWorthAccountsUrl = () => {
  return `/api/net-worth/sync`;
};

export const syncNetWorthAccounts = async (
  options?: RequestInit,
): Promise<SyncNetWorthAccounts200> => {
  return customFetch<SyncNetWorthAccounts200>(getSyncNetWorthAccountsUrl(), {
    ...options,
    method: "POST",
  });
};

export const getSyncNetWorthAccountsMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof syncNetWorthAccounts>>,
    TError,
    void,
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof syncNetWorthAccounts>>,
  TError,
  void,
  TContext
> => {
  const mutationKey = ["syncNetWorthAccounts"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof syncNetWorthAccounts>>,
    void
  > = () => {
    return syncNetWorthAccounts(requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type SyncNetWorthAccountsMutationResult = NonNullable<
  Awaited<ReturnType<typeof syncNetWorthAccounts>>
>;

export type SyncNetWorthAccountsMutationError = ErrorType<unknown>;

/**
 * @summary Sync derived account balances from transactions
 */
export const useSyncNetWorthAccounts = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof syncNetWorthAccounts>>,
    TError,
    void,
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationResult<
  Awaited<ReturnType<typeof syncNetWorthAccounts>>,
  TError,
  void,
  TContext
> => {
  return useMutation(getSyncNetWorthAccountsMutationOptions(options));
};

/**
 * @summary Save a net worth snapshot for history tracking
 */
export const getTakeNetWorthSnapshotUrl = () => {
  return `/api/net-worth/snapshot`;
};

export const takeNetWorthSnapshot = async (
  options?: RequestInit,
): Promise<NetWorthSnapshotRecord> => {
  return customFetch<NetWorthSnapshotRecord>(getTakeNetWorthSnapshotUrl(), {
    ...options,
    method: "POST",
  });
};

export const getTakeNetWorthSnapshotMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof takeNetWorthSnapshot>>,
    TError,
    void,
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  Awaited<ReturnType<typeof takeNetWorthSnapshot>>,
  TError,
  void,
  TContext
> => {
  const mutationKey = ["takeNetWorthSnapshot"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey }, request: undefined };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof takeNetWorthSnapshot>>,
    void
  > = () => {
    return takeNetWorthSnapshot(requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type TakeNetWorthSnapshotMutationResult = NonNullable<
  Awaited<ReturnType<typeof takeNetWorthSnapshot>>
>;

export type TakeNetWorthSnapshotMutationError = ErrorType<unknown>;

/**
 * @summary Save a net worth snapshot for history tracking
 */
export const useTakeNetWorthSnapshot = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof takeNetWorthSnapshot>>,
    TError,
    void,
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationResult<
  Awaited<ReturnType<typeof takeNetWorthSnapshot>>,
  TError,
  void,
  TContext
> => {
  return useMutation(getTakeNetWorthSnapshotMutationOptions(options));
};
